...
MySidebar.searchFunc = {
    ...
    search: function search(where, key, link) {
        key = encodeURIComponent(key);
        var _self = MySidebar.searchFunc;
        var thisUrl;
        switch (where) {
            case "side_google_search":
                thisUrl = "https://www.google.com/search?q=" + key;
                break;
            ...
            case "side_image_search":
                ...
                if (myImg) {
                    ...
                    myfText = encodeURIComponent(myfText);
                    thisUrl = "https://www.google.com/search?tbm=isch&q=" + myfText;
                }
                break;
            default:
                //communicate to get link
                thisUrl = link.replace("%s", key);

        }
        var Communicate = new MySidebar.dndcommunicate.sendRequest({url: thisUrl, method: "SidebarSearch", arguments: {key: key, where: where}, directCall: 1}, function() {
        });
    },
    share: function share(where, key, link) {
        var thisUrl;
        var _self = MySidebar.searchFunc;
        switch (where) {
            case "side_facebook":
        ...
        var Communicate = new MySidebar.dndcommunicate.sendRequest({url: thisUrl, method: "SidebarShare", arguments: {key: key, where: where}, directCall: 1}, function() {
        });
    }
